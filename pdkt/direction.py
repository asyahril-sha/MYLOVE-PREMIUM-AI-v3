...
"""
=============================================================================
MYLOVE PREMIUM AI - PDKT DIRECTION SYSTEM
=============================================================================
...
class DirectionSystem:
    ...
    def format_direction_info(self, pdkt_id: str) -> str:
        """Format direction info untuk display"""
        data = self.get_direction(pdkt_id)
        if not data:
            return "Arah PDKT belum ditentukan"
        
        direction = data['direction']
        hint = self.get_hint(pdkt_id)
        intensity = data['intensity']
        
        intensity_bar = "🔥" * intensity + "⚪" * (10 - intensity)
        
        lines = [
            f"**Arah PDKT:** {self.get_direction_text(pdkt_id)}",
            f"**Intensitas:** {intensity_bar} ({intensity}/10)",
            f"**Hint:** {hint}",
            f"**Riwayat perubahan:** {data['times_changed']} kali"
        ]
