...
"""
=============================================================================
MYLOVE PREMIUM AI - PDKT LIST FORMATTER
=============================================================================
...
class PDKTListFormatter:
    ...
    def format_pdkt_detail(self, pdkt: Dict, inner_thoughts: List[str] = None) -> str:
        ...
        if inner_thoughts:
            ...
            for thought in inner_thoughts[-3:]:
        ...
        # Command tips
        lines.append("💡 **Command:**")
        lines.append(f"• `/pdktwho {pdkt.get('pdkt_id', '')}` - Lihat arah PDKT")
        if pdkt.get('is_paused', False):
            lines.append(f"• `/resumepdkt {pdkt.get('pdkt_id', '')}` - Resume PDKT")
        else:
            lines.append(f"• `/pausepdkt {pdkt.get('pdkt_id', '')}` - Pause PDKT")
        lines.append(f"• `/stoppdkt {pdkt.get('pdkt_id', '')}` - Hentikan PDKT")
