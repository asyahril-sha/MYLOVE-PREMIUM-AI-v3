...
"""
=============================================================================
MYLOVE PREMIUM AI - CHEMISTRY SYSTEM (99% REALISME)
=============================================================================
...
class ChemistryScore:
    ...
    def get_trend(self) -> str:
        """Dapatkan tren chemistry (naik/turun/stabil)"""
        if len(self.history) < 3:
            return "stabil"
        
        recent = self.history[-3:]
        avg_change = sum(h['change'] for h in recent) / 3
...
class ChemistrySystem:
    ...
    def get_trend(self, pdkt_id: str) -> str:
        """Dapatkan tren chemistry"""
        chemistry = self.get_chemistry(pdkt_id)
        if chemistry:
            return chemistry.get_trend()
        return "stabil"
    ...
    def format_chemistry_info(self, pdkt_id: str) -> str:
        """Format chemistry info untuk ditampilkan (tanpa angka)"""
        chemistry = self.get_chemistry(pdkt_id)
        if not chemistry:
            return "Belum ada chemistry"
        
        vibe = chemistry.get_vibe()
        desc = chemistry.get_description()
        trend = chemistry.get_trend()
        ...
        return f"{vibe}\n{desc}\n{trend_emoji} Tren: {trend}"


__all__ = ['ChemistrySystem', 'ChemistryLevel', 'ChemistryScore']
