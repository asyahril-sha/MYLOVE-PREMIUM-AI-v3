...
"""
=============================================================================
MYLOVE PREMIUM AI - BACKUP RECOVERY
=============================================================================
...
class RecoveryManager:
    ...
    async def restore_backup(self, backup_path: Path, dry_run: bool = False) -> Dict:
        ...
        try:
            ...
            return {
                "success": True,
                "filename": backup_path.name,
                "files_restored": len(verify_result.get('files', [])),
                "size_mb": verify_result['size_mb'],
    ...
    async def _restore_files(self, temp_dir: Path):
        ...
        # Restore config (without overwriting secrets)
        config_backup = temp_dir / "config"
        if config_backup.exists():
            config_dir = Path("config")
            if config_dir.exists():
                # Only restore non-sensitive config
                config_file = config_backup / "config.json"
                if config_file.exists():
                    shutil.copy2(config_file, config_dir / "config_backup.json")
                    logger.info("✅ Config restored (as backup)")
    ...
    async def get_restore_stats(self) -> Dict:
        """Get restore statistics"""
        backups = await self.list_backups(50)
