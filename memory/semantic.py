...
"""
=============================================================================
MYLOVE PREMIUM AI - SEMANTIC MEMORY
=============================================================================
...
class SemanticMemory:
    ...
    def __init__(self):
        ...
        logger.info("✅ SemanticMemory initialized (PERMANENT storage)")
        ...
        for category, patterns in self.fact_patterns.items():
            for pattern, fact_type in patterns:
                ...
                if match:
                    ...
                    logger.debug(f"Extracted fact: {category}.{fact_type} = {value}")
    ...
    async def update_preference(self,
        ...
        """
        current = self.preferences[user_id][role].get(pref_type, {}).get(item, 0.5)
        ...
        # Simpan
        if pref_type not in self.preferences[user_id][role]:
            self.preferences[user_id][role][pref_type] = {}
        
        self.preferences[user_id][role][pref_type][item] = {
            'score': new_score,
            'count': self.preferences[user_id][role][pref_type].get(item, {}).get('count', 0) + 1,
            'last_updated': time.time(),
            'last_context': context
        }
        
        logger.debug(f"Preference updated: user {user_id} {role} {pref_type} {item} -> {new_score:.2f}")
    ...
    async def delete_user_data(self, user_id: int):
        ...
        if user_id in self.habits:
            del self.habits[user_id]
        
        logger.info(f"Deleted all semantic data for user {user_id}")


__all__ = ['SemanticMemory', 'FactCategory', 'PreferenceType']
