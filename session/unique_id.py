...
"""
=============================================================================
MYLOVE PREMIUM AI - UNIQUE ID GENERATOR
=============================================================================
...
- Support untuk berbagai tipe ID (PDKT, MANTAN, FWB, HTS, TEMP)
=============================================================================
"""
...
class UniqueIDGenerator:
    ...
    def generate_v2(self, bot_name: str, role: str, user_id: int) -> str:
        """
        Generate unique ID dengan format V2:
        MYLOVE-{NAMA_BOT}-{ROLE}-{USERID}-{YYYYMMDD}-{SEQ}
        
        Args:
            bot_name: Nama bot yang dipilih (contoh: "Sari", "Dewi")
        ...
        nama_upper = bot_name.upper().replace(' ', '_').replace('-', '_')
    ...
    def generate_v1(self, role: str, user_id: int) -> str:
        ...
        user_str = str(user_id)
        date_str = datetime.now().strftime("%Y%m%d")
        seq = self._get_next_sequence_v1(role_upper, user_str, date_str)
        seq_str = f"{seq:03d}"
    ...
    def generate_pdkt_id(self, bot_name: str, role: str, user_id: int) -> str:
        ...
        Format: PDKT-{NAMA_BOT}-{ROLE}-{USERID}-{TIMESTAMP}-{RANDOM}
    ...
    def generate_mantan_id(self, bot_name: str, role: str, user_id: int) -> str:
        ...
        Format: MANTAN-{NAMA_BOT}-{ROLE}-{USERID}-{TIMESTAMP}-{RANDOM}
        """
        bot_upper = bot_name.upper().replace(' ', '_')
        role_upper = role.upper()
        timestamp = int(time.time())
    ...
    def generate_fwb_id(self, bot_name: str, role: str, user_id: int) -> str:
        ...
        Format: FWB-{NAMA_BOT}-{ROLE}-{USERID}-{TIMESTAMP}-{RANDOM}
        """
        bot_upper = bot_name.upper().replace(' ', '_')
        role_upper = role.upper()
        timestamp = int(time.time())
    ...
    def generate_hts_id(self, bot_name: str, role: str, user_id: int) -> str:
        ...
        Format: HTS-{NAMA_BOT}-{ROLE}-{USERID}-{TIMESTAMP}-{RANDOM}
        """
        bot_upper = bot_name.upper().replace(' ', '_')
        role_upper = role.upper()
        timestamp = int(time.time())
    ...
    def _get_next_sequence(self, bot_name: str, role: str, user_id: str, date: str) -> int:
        """Dapatkan sequence number untuk format V2"""
        key = f"{bot_name}_{role}_{user_id}_{date}"
        return self._get_next_sequence_key(key)
    
    def _get_next_sequence_v1(self, role: str, user_id: str, date: str) -> int:
        """Dapatkan sequence number untuk format V1"""
    ...
    def parse(self, unique_id: str) -> Optional[Dict]:
        ...
        Support V1, V2, dan semua tipe ID
        
        Args:
            unique_id: String ID 
                       Format V2: MYLOVE-SARI-IPAR-12345678-20240319-001
                       Format V1: MYLOVE-IPAR-12345678-20240315-001
                       Format PDKT: PDKT-SARI-IPAR-12345678-1647358291-ABC1
    ...
    def get_session_summary(self, unique_id: str) -> str:
        ...
        if parsed.get('type') == 'session_v2' and parsed.get('bot_name'):
            return f"Session dengan {parsed['bot_name'].title()} ({parsed['role'].title()}) - {parsed['date']}"
        else:
    ...
    def reset_counter(self):
        ...
        self.counter = {}
        logger.info("Sequence counter reset")
