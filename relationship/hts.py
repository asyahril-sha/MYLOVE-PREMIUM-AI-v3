...
class HTSManager:
    ...
    async def get_hts_by_name(self, user_id: int, bot_name: str) -> Optional[Dict]:
        """
        Dapatkan HTS berdasarkan nama bot
        ...
        for hts_id, hts in self.hts_relations[user_id].items():
            if hts['bot_name'].lower() == bot_name.lower():
                return hts
    ...
    async def format_hts_list(self, user_id: int, show_all: bool = False) -> str:
        ...
        if not hts_list:
            return "Belum ada HTS. Selesaikan hubungan dengan role NON-PDKT sampai level 12 dan /close untuk mendapatkan HTS."
