...
"""
=============================================================================
MYLOVE PREMIUM AI - RISK CALCULATION SYSTEM
=============================================================================
...
class RiskCalculator:
    ...
    async def calculate_risk(
        ...
        if event:
            if 'risk_increase' in event:
                ...
                event_effect['event_name'] = event['name']
        ...
        return {
            ...
            "event": event_effect if event_effect else None
        }
    ...
    def format_risk_report(self, risk_data: Dict, location_name: str) -> str:
        ...
        if risk_data.get('event'):
            ...
            lines.append("")
            lines.append(f"🎲 **Random Event:** {event.get('event_name', '')}")
            if 'risk_change' in event:
                if event['risk_change'] > 0:
                    lines.append(f"⚠️ Risk +{event['risk_change']}%")
                else:
                    lines.append(f"✅ Risk {event['risk_change']}%")
    ...
    def get_risk_multiplier(self, risk_level: int) -> float:
        """Dapatkan multiplier untuk thrill berdasarkan risk"""
        if risk_level < 30:
            return 0.5
        elif risk_level < 50:
            return 0.8
    ...
    def get_risk_description(self, risk_level: int) -> str:
        """Dapatkan deskripsi risk level"""
        if risk_level < 20:
            return "Sangat aman, hampir tidak ada risiko"
        elif risk_level < 40:
            return "Cukup aman, masih nyaman"
        elif risk_level < 60:
            return "Risiko sedang, perlu waspada"
        elif risk_level < 80:
            return "Risiko tinggi, harus cepat"
        else:
            return "Risiko sangat tinggi, sangat berbahaya"
